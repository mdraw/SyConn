...
def detect_cs(arr):
    ...
    cdef uint32_t[:, :, :] arrC = arr
    cdef uint32_t[:, :, :] edgesC = edges
    cs_seg = process_block_nonzero(edgesC, arrC, (13, 13, 7))
...
def _extract_agg_cs_thread(args):
    ...
    for chunk in chunk_block:
        ...
        indices = itertofrom_overlaycubes_to_matrixols.product(range(dims[0]), range(dims[1]),
                                    range(dims[2]))
